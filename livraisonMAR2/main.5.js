...
requirejs(['ModulesLoaderV2.js'], function()
		{ 
			...
			ModulesLoader.requireModules([ "myJS/ThreeRenderingEnv.js", 
			                              ...
			                              "myJS/navZ.js",
			                              "FlyingVehicle.js","helicoIA.js",
			                              "Util.js","helico.js","ParticleSystem.js","bezier.js",
			                              "particle.js","MathExt.js","Interpolators.js","camera.js"]) ;
...
//	renderingEnvironment.camera.rotation.x = 85.0*3.14159/180.0 ;
		//////////////////////////////////////::
			var sc= new Scene3D(renderingEnvironment,Lights,Loader);
				var u = new Util3D();
	var helicopilot = new helicopter(sc);
u.add_mesh ( sc,helicopilot.mesh);
u.scale(helicopilot.mesh, 1.5,1.5,1.5);
 u.trY (helicopilot.mesh,  -14);
 u.trX (helicopilot.mesh,  -254);
var eehepilotg =  new EngineExhaust(sc);
var eehepilotd =  new EngineExhaust(sc);
var helicopilotIA = new HelicopilotIA(helicopilot, eehepilotg,eehepilotd);
 
	var helicoauto = new helicopter(sc);
u.add_mesh ( sc,helicoauto.mesh);
u.scale(helicoauto.mesh, 1.5,1.5,1.5);
 u.trY (helicoauto.mesh,  -14);
 u.trX (helicoauto.mesh, -204);
 u.trZ (helicoauto.mesh, 204);

var eeheautog =  new EngineExhaust(sc);
var eeheautod =  new EngineExhaust(sc);
var helicoautoIA = new HelicoautoIA(helicoauto, eehepilotg,eehepilotd);
 
	var cam_man= new CameraManager (sc,vehicle, helicoautoIA,helicopilotIA);
	cam_man.set_mode_auto();
...
function handleKeyPress (k){
	 //console.log("zzzzzzzzzzzzzzzzzzz"+k.keycode);
...
console.log("----------------------"+vehicle.position.x+"/"+vehicle.position.y);
		...
		cam_man.onpress(69);
			...
			if (currentlyPressedKeys[71]) // (G) Down 
		{
cam_man.set_mode_auto();
		}
				if (currentlyPressedKeys[86]) // (V) Down 
		{
cam_man.set_mode_vehicle();
		}
				if (currentlyPressedKeys[72]) // (H) Down 
		{
cam_man.set_mode_helicoauto();
		}
					if (currentlyPressedKeys[74]) // (J) Down 
		{
cam_man.set_mode_helicopilot();
		}
	}
	...
	function render() { 
		...
		renderingEnvironment.camera.position.z = NAV.z+150+vehicle.speed.length()*2 ;
		console.log(vehicle.speed.z) ;
//		renderingEnvironment.camera.rotation.z = vehicle.angles.z-Math.PI/2.0 ;
		// Rendering
		vehicle.update_position();
		helicopilotIA.update_position();
cam_man.setup_camera();
 cam_man.look();
